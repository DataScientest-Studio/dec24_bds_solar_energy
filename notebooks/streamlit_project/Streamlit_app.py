...
st.title("dec24_bds_solar_energy")
...
if page == pages[2]:
    ...
    #st.matplotlib.pyplot.show(block=False)
...
X_Train = [X_train_os_robust, X_train_us_robust, X_train_os_std]
X_Test = [X_test_robust, X_test_robust, X_test_std]
y_Train = [y_train_os, y_train_us, y_train_os]
Label = ['Oversampled Robust scaled', 'Undersampled Robust Scaled', 'Oversampled Standard scaled']
...
for X_train, X_test, y_train, label in zip(X_Train, X_Test, y_Train, Label):
    #st.write(f'#### {label}')
    best_models = {}
    ...
    for name, model in models.items():
        ...
        #st.write(f"Best {name} Params: {grid_search.best_params_}")
        best_model.append([name,grid_search.best_params_])
    # Evaluate the best models on train and test sets
    for name, model in best_models.items():
        ...
        # Compute ratio
        f1_ratio = f1_test / f1_train if f1_train != 0 else 0
            
        results.append([name, label, f1_train, f1_test, f1_ratio])
    
# Create DataFrame for final results table
results_df = pd.DataFrame(results, columns=['Model', 'Data Type', 'Train F1-Score', 'Test F1-Score', 'Test/Train F1-Score Ratio'])
...
if page == pages[6]:
    ...
    st.write('### Best Model and Metrics')
...
background_data = shap.kmeans(X_train_os_robust, 10) # reduce to 1 to make it faster
...
shap.summary_plot(shap_values[1], X_test_robust)  # For class 1 (modify based on your model output)
